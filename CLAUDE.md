...
- **`data-<city>.json`** (nyc, brooklyn, queens, flushing, ac, baltimore, dc, pittsburgh, richmond, boston, …) — one dataset file per additional city, fetched lazily on first switch and cached in memory. The authoritative city list is `CONFIG.cities` in `index.html`.
